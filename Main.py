...
def generate_report(sales_data, inventory_data):
    ...
    for sku in inventory_data['Part No.']:
        ...
        reorder_qty = max(forecast_30_qty - (inventory_data.loc[inventory_data['Part No.'] == sku, 'In stock'].values[0] + inbound_qty), 0)

        forecast_report.append([product_name, sku, velocity, forecast_30_qty, inventory_need_30])
        
        if reorder_qty > 0:
            reorder_report.append([product_name, sku, inbound_qty, reorder_qty, forecast_30_qty])

    forecast_df = pd.DataFrame(forecast_report, columns=['Product', 'SKU', 'Sales Velocity', 'Forecast Sales Qty (30 Days)', 'Forecast Inventory Need (30 Days)'])
